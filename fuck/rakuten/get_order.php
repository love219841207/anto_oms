...
// if(isset($_GET['list_orders'])){
  ...
  //   $signatureversion = $res['signatureversion'];
    $serviceSecret = 'SP266521_fBKi6O9ahPfT3ISW';
	$licenseKey         		= 'SL266521_ZKayAawDUELnvjvF';					//*

  //   $marketplaceid_id_1 = $res['marketplaceid_id_1'];

 	$param = array();
	$param['serviceSecret']   		= 'SP266521_fBKi6O9ahPfT3ISW';			//*
	$param['itemUrl']         		= 'Z-CFILM-TEGL--2385';					//*
	// $Authorization         		= 'serviceSecret:SP266521_fBKi6O9ahPfT3ISW';					//*
	...
	// 	  )
	// );
// $response = $this->getClient()->currentResponse();
// $result = simplexml_load_string($response['body']);

// 	$arr   = implode('&', $url);

// 	$sign  = 'GET' . "\n";
// 	$sign .= 'https://api.rms.rakuten.co.jp' . "\n";
// 	$sign .= 'es/1.0/item/get' . "\n";
// 	$sign .= $arr;

	// $signature = hash_hmac("sha256", 'ESA ' .$serviceSecret, $licenseKey, true);
	$signature = 'ESA ' . base64_encode($serviceSecret . ':' . $licenseKey);
	$link  = "https://api.rms.rakuten.co.jp/es/1.0/item/get?itemUrl=Z-LDFS-H1-GY-F";
	$link .= "&Authorization=" . $signature;
