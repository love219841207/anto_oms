...
if(isset($_POST['store'])){
    ...
    //读取店铺配置
    $sql = "SELECT * FROM conf_Yahoo WHERE store_name = '{$store}'";
    $res = $db->getOne($sql);
    ...
    // 按照合单号发送
    $sql = "SELECT send_id FROM yahoo_response_list WHERE order_id IN ({$order_items})";
    $res = $db->getAll($sql);
    $sss = array();
    ...
    foreach ($sss as $keyvalue) {
        ...
        if($mail_tpl == 'send_express'){    
            // 如果是发货通知信
            $sql = "UPDATE yahoo_express SET over_mail = 1 WHERE yahoo_order_id IN ($order_items)";
            $res = $db->execute($sql);
        ...
        //读取邮箱、收件人等信息
        $sql = "SELECT * FROM yahoo_response_list WHERE send_id = '{$value}'";
        $res = $db->getOne($sql);
        ...
        $sql = "SELECT sum(order_tax) as order_tax,sum(points) as points,sum(coupon) as coupon,shipping_price FROM yahoo_response_list WHERE send_id = '{$value}'";
        $res = $db->getOne($sql);
        ...
        // 读取购买信息
        // 查询该send_id下的订单
        $sql = "SELECT order_id FROM yahoo_response_list WHERE send_id = '{$value}'";
        $res = $db->getAll($sql);
        $now_order_id = '';
        foreach ($res as $value) {
        ...
        $sql = "SELECT * FROM yahoo_response_info WHERE order_id IN ({$now_order_ids})";
        $res = $db->getAll($sql);
        $goods_money = 0;
        foreach ($res as $val) {
