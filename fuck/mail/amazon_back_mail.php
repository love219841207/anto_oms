...
if(isset($_POST['store'])){
	...
	foreach ($sss as $keyvalue) {
		...
		if($mail_tpl == 'send_express'){	
			// 如果是发货通知信
			$sql = "UPDATE amazon_express SET over_mail = 1 WHERE amazon_order_id IN ($order_items)";
			$res = $db->execute($sql);
    	...
    	$receive_name = preg_replace('/\[.*?\]/', '', $receive_name);
	 	...
	 	$sql = "SELECT sum(order_tax) as order_tax,sum(points) as points,sum(coupon) as coupon,shipping_price FROM amazon_response_list WHERE send_id = '{$value}'";
	 	$res = $db->getOne($sql);
