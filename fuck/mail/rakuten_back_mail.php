...
if(isset($_POST['store'])){
    ...
    //读取店铺配置
    $sql = "SELECT * FROM conf_Rakuten WHERE store_name = '{$store}'";
    $res = $db->getOne($sql);
    ...
    foreach ($sss as $keyvalue) {
        ...
        if($mail_tpl == 'send_express'){    
            // 如果是发货通知信
            $sql = "UPDATE rakuten_express SET over_mail = 1 WHERE rakuten_order_id IN ($order_items)";
            $res = $db->execute($sql);
        ...
        //读取邮箱、收件人等信息
        $sql = "SELECT * FROM rakuten_response_list WHERE send_id = '{$value}'";
        $res = $db->getOne($sql);
        ...
        $sql = "SELECT sum(order_tax) as order_tax,sum(points) as points,sum(coupon) as coupon,sum(shipping_price) as shipping_price FROM rakuten_response_list WHERE send_id = '{$value}'";
        $res = $db->getOne($sql);
        ...
        // 读取购买信息
        // 查询该send_id下的订单
        $sql = "SELECT order_id FROM rakuten_response_list WHERE send_id = '{$value}'";
        $res = $db->getAll($sql);
        $now_order_id = '';
        foreach ($res as $value) {
        ...
        $sql = "SELECT * FROM rakuten_response_info WHERE order_id IN ({$now_order_ids})";
        $res = $db->getAll($sql);
        $goods_money = 0;
        foreach ($res as $val) {
