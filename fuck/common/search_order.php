...
if(isset($_POST['ready_send_data'])){
    ...
    if($search_field == ''){   //0没有筛选条件
        $sql = "SELECT * FROM send_table ORDER BY send_id DESC LIMIT {$start},{$page_size}";
    }else{
        $sql = "SELECT * FROM send_table WHERE {$search_field} LIKE '%{$search_key}%' ORDER BY send_id DESC LIMIT {$start},{$page_size}";
    }
    $res = $db->getAll($sql);
    echo json_encode($res);
}