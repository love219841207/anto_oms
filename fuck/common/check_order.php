...
// 11111111111111111111 验证邮编地址开始 11111111111111111111
	...
	$sql = "SELECT $order_id as order_ids FROM $response_list WHERE store = '{$store}' AND post_ok = 0";
	$res = $db->getAll($sql);

	foreach ($res as $value) {
		$now_order_id = $value['order_ids'];
		// 查询客人填写的post和addr
...
// 22222222222222222222 检测电话是否填写开始 22222222222222222222
	...
	$sql = "SELECT $order_id as order_ids FROM $response_list WHERE store = '{$store}' AND tel_ok = 0";
	$res = $db->getAll($sql);
	foreach ($res as $value) {
		$now_order_id = $value['order_ids'];
...
// 33333333333333333333 检测SKU 如果是福袋则拆开始 33333333333333333333
	...
	$sql = "SELECT $order_id AS order_ids FROM $response_list WHERE store = '{$store}' AND sku_ok = 0";
	$res = $db->getAll($sql);
	foreach ($res as $value) {
		$now_order_id = $value['order_ids'];
		// 查询总item 数
...
// 44444444444444444444 运费代码验证开始 44444444444444444444
	if($station == 'amazon'){
		...
		$sql = "UPDATE $response_list SET yfcode='宅配便' WHERE yfcode_ok = 0";
		$res = $db->execute($sql);
