app.controller('yahooCtrl', ['$scope','$state','$http','$log','$timeout', function($scope,$state,$http,$log,$timeout){
    //赋日期
    var myDate = new Date();
    ...
    $scope.check_items = function(){
        ...
        angular.forEach($scope.express_list, function(value, index){
            if($scope.express_list[index].is_click == true){
                my_checked.push("'"+$scope.express_list[index].yahoo_order_id+"'");
            }
        });
    ...
    $scope.get_express_list = function(){
        ...
        $http.get('/fuck/yahoo/yahoo_send_express.php', {
            params:{
    ...
    $scope.down_express_xlsx = function(){
        ...
        $http.post('/fuck/yahoo/yahoo_send_express.php', post_data).success(function(data) {
            if(data == 'ok'){
                window.location="/down/yahooExpress.csv";
            }else{
                $log.info(data);
    ...
    $scope.read_error_mail = function(){
        $http.get('/fuck/mail/yahoo_send_mail.php', {
            params:{
